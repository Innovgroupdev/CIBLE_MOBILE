  ...
  void initState() {
   ...
   getLieuxFromAPI() async {
     ...
     await ParametreCategorieDBcontroller().liste().then((value) {
      ...
      setState(() {
        value.forEach((element) {
          categorieIdSelected.add(element['id_categorie']);
        });
      });
      print('zzzzzzzzzzzzz2'+categorieIdSelected.toString());
    });
    ...
    await ParametreLieuDBcontroller().liste().then((value) {
      ...
      setState(() {
        value.forEach((element) {
          _selectedLieu.add(element['ville']);
        });
      });
      print('zzzzzzzzzzzzz4'+_selectedLieu.toString());
    });
    List data = [];
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      print('ddddddddddda'+response.body.toString());
      setState(() {
                      ...
                      return InkWell(
                        onTap: (() {
                          setState(() {
                            ...
                            if(_selectedLieu.contains(_data![index]['lieu']))
                            {
                              _selectedLieu.remove(_data![index]['lieu']);
                            }else{
                              _selectedLieu.add(_data![index]['lieu']);
                            }
                              });
                              print('_selectedLieuuuuuuu'+_selectedLieu.toString());
                        }),
                        child: Container(
                          decoration: BoxDecoration(
                            color: 
                            _selectedLieu.contains(_data![index]['lieu'])?
                    ...
                    child: GridView.builder(
                      ...
                      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                          ...
                          crossAxisCount: MediaQuery.of(context).orientation ==
                              ...
                              : 4),
                      itemBuilder: (BuildContext context, int index) {
                        if(categorieIdSelected.contains(_categories[index].id)){
                          _categories[index].checked = true;
                        }
                        return GestureDetector(
                          onTap: () {
                            setState(() {
                              ...
                              // }
                              print('stateeeeeeeee1'+_categories[index].checked.toString());
                              _categories[index].changeEtat();
                               print('stateeeeeeeee2'+_categories[index].checked.toString());
                              if (_categories[index].checked && !categorieIdSelected.contains(_categories[index])) {
                                
                              categorieIdSelected.add(_categories[index].id) ;
                                // Provider.of<EventCreateProvider>(context,
                                //         listen: false)
                                //     .categorie = _categories[index];
                              }else{
                                categorieIdSelected.remove(_categories[index].id);
                              }
                              print("categorieIdSelecteddddddddd"+categorieIdSelected.toString());
                            });
                          },
                          child: Stack(
                            children: [
                              ...
                              Positioned(
                                  ...
                                  child: Container(
                                    ...
                                    decoration: BoxDecoration(
                                        border: Border.all(
                                            color: _categories[index].checked || categorieIdSelected.contains(_categories[index].id)
                                                ? appColorProvider.primaryColor1
                                                : const Color.fromARGB(
                                                    31, 151, 151, 151)),
                                        color: _categories[index].checked || categorieIdSelected.contains(_categories[index].id)
                                            ? const Color.fromARGB(255, 76, 207, 24)
                                            : const Color.fromARGB(
                                                106, 245, 245, 245),
                    ...
                    RaisedButtonDecor(
                      onPressed: () async {
                        if(categorieIdSelected != null && _selectedLieu != null && _selectedLieu != ''){
                          categorieIdSelected.forEach((cate) async{
                            await ParametreCategorieDBcontroller().insert(cate);
                          });
                          _selectedLieu.forEach((lieu) async{
                            await ParametreLieuDBcontroller().insert(lieu);
                          });
