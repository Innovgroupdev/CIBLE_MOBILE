...
getAllEventUserPassFromAPI() async {
    ...
    var response = await http.get(
      Uri.parse('$baseApiUrl/events/totalparticipated'),/*surveys/users/active*/
      headers: {
...
getAllSurveyRespondedFromAPI() async {
  ...
  getAllGadgetsUserPassFromAPI() async {
    var token = await SharedPreferencesHelper.getValue('token');
    var response = await http.get(
      Uri.parse('$baseApiUrl/gadgetspurchased'),/*surveys/users/active*/
      headers: {
        "Accept": "application/json",
        "Content-Type": "application/json",
        'Authorization': 'Bearer $token',
   ...
   getAllEventsArchivedUserFromAPI() async {
    var token = await SharedPreferencesHelper.getValue('token');
    var response = await http.get(
      Uri.parse('$baseApiUrl/userpart/events/archivedforpart'),
      headers: {
        "Accept": "application/json",
        "Content-Type": "application/json",
        'Authorization': 'Bearer $token',
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      // eventsList = jsonDecode(response.body)['events'];
      setState(() {
        sondages = getEventFromMap(jsonDecode(response.body)['data'] as List,{});
   ...
   getUserEventsSuggestionsFromAPI() async {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      // eventsList = jsonDecode(response.body)['events'];
      setState(() {
        suggestions = getEventFromMap(jsonDecode(response.body)['data'] as List,{});
    ...
    var token = await SharedPreferencesHelper.getValue('token');
    var response = await http.get(
      Uri.parse('$baseApiUrl/userpart/events/past'),/*surveys/users/active*/
      headers: {
        "Accept": "application/json",
        "Content-Type": "application/json",
        'Authorization': 'Bearer $token',
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      // eventsList = jsonDecode(response.body)['events'];
      setState(() {
        eventsPassed = getEventFromMap(jsonDecode(response.body)['data'] as List,{});
   ...
   getComingEventsFromAPI() async {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      setState(() {
        eventsComing = getEventFromMap(jsonDecode(response.body)['data'] as List,{});
       // eventsComingTickets = getEventFromMap(jsonDecode(response.body)['data'] as List,{});
  ...
  for (var element in madDecode) {
    var event = Event1.fromMap(element['event']??element['evenement']??element /*, map*/);
    tagObjs.add(event);
  }
  return tagObjs;
}
    ...
    return Consumer<AppColorProvider>(
      ...
      return Scaffold(
          ...
          body: Container(
            ...
            child:
            sondages == null || suggestions == null ||
            solde == null || devises.isEmpty || 
            eventsComing == null || eventsPassed == null  ||
             allEventsPassed == null || allGadgetsPassed == null?
                            ...
                            InkWell(
                              ...
                              child: Container(
                                ...
                                Container(
                                          ...
                                          0
                                      ?
                                      Colors.red:appColorProvider.black54
                                      ,
                                            badgeContent:
                      ...
                      child: Listener(onPointerDown: (details) {
                       ...
                       // setState(() {});
                        ...
                        return TabBarView(
                          ...
                          eventsPassed== null?
                          const Center(child: CircularProgressIndicator(),): 
                          EventsActifs(type:'Passés',eventList: eventsPassed!),
                          //child: Satistics(),
                        ),
                         SizedBox(
                          child: 
                          eventsComing == null?
                          const Center(child: CircularProgressIndicator(),):
                          EventsActifs(type:'Avenir',eventList: eventsComing!),
                          //     Center(
                          //   child: Text('vide2'),
                          // ),
                        ),
                         SizedBox(
                          ...
                          suggestions == null?
                          const Center(child: CircularProgressIndicator(),):
                          EventsActifs(type:'Suggestions',eventList: suggestions!),
                          // child: Center(
                          //   child: Text('vide3'),p
                          // ),
                        ),
                          ...
                          sondages == null?
                          const Center(child: CircularProgressIndicator(),): 
                          EventsActifs(type:'Sondages',eventList: sondages!),
                          //child: Satistics(),
                        ),
                          ],
                        );
